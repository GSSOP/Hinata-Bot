...
client.on("ready", () => {
    ...
    console.log("automod is ready join support server https://discord.gg/5hUj23ApDJ")
});
client.on("message", async message => {
if(!message.guild) return;
...
console.log("Subscribe on Viral Matrix https://www.youtube.com/channel/UCO6Ka3cpz74nIbwNNmGffQQ")
...
client.on("message", async message => {
  if (message.channel.name == "abotchat") {
    ...
    message.channel.startTyping();
    if (!message.content) return message.channel.send("Please say something.");
    scb.chat({message: message.content, name: client.user.username, owner:"ViralMatrix", user: message.author.id, language:"auto"}).then(reply => {
